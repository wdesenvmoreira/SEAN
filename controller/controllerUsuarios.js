...
const findAll = async() =>{
    ...
    try {
        return await knex('usuarios').select('id_usuario', 'usuario', 'master', 'status')
    } catch (error) {
        return error
    }
...
const findById = async(id) =>{
    try {
           const registro = knex('usuarios')
                            .where('id_usuario', id)
                            .select('id_usuario', 'usuario', 'master', 'status')
                            .first()     
            return registro
    } catch (error) {
        return error
...
const update = async(id_usuario, dados) => {   
    if(dados.senha){
        const salt = bcrypt.genSaltSync()
        dados.senha = bcrypt.hashSync(dados.senha, salt)
    }
    console.log('id_ususario:', id_usuario, '. dados:', dados)
    try {
        return await knex('usuarios')
                .where({ id_usuario })
                .update({...dados})
...
const deletar = async(id_usuario) =>{
    if(id_usuario != 1){
        try {
        return await knex('usuarios')
                        .where({ id_usuario })
